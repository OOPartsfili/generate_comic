...
test('首屏与窄窗口布局、设置、示例和历史恢复', async ({ page }, info) => {
  ...
  await expect(page.getByText(/当前插画调度：gpt-5.5/)).toBeVisible();
  await expect(page.getByText('官方 Codex 版本：0.138.0')).toBeVisible();
  const diagnosticDownload = page.waitForEvent('download');
  await page.getByRole('button', { name: '导出诊断日志' }).click();
  const diagnosticFile = await diagnosticDownload;
  const diagnostic = JSON.parse(await readFile(await diagnosticFile.path(), 'utf8'));
  expect(diagnostic.format).toBe('moge-diagnostics-v1');
  expect(diagnostic.entries).toBeInstanceOf(Array);
  ...
  await page.screenshot({ path: info.outputPath('06-codex-connection.png') });
