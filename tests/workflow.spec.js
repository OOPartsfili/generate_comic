...
test('首屏与窄窗口布局、设置、示例和历史恢复', async ({ page }, info) => {
  ...
  await page.getByLabel('生成方式').selectOption('codex');
  await expect(page.getByLabel('OpenAI API 密钥')).toBeHidden();
  await page.getByRole('button', { name: '保存并检测连接' }).click();
  await expect(page.getByText(/ChatGPT 已连接 · PRO/)).toBeVisible();
  await expect(page.getByText(/剩余 80%/)).toBeVisible();
  await page.getByLabel('Codex 文字模型').selectOption('gpt-5.5');
  await page.screenshot({ path: info.outputPath('06-codex-connection.png'), fullPage: true });
  await page.getByLabel('生成方式').selectOption('api'); await expect(page.getByLabel('OpenAI API 密钥')).toHaveAttribute('type', 'password');
  await page.getByRole('button', { name: '保存并检测连接' }).click();
