...
if (!locked) app.quit();
  ...
  app.whenReady().then(async () => {
    ...
    service = await startServer({ port: 0, token, dataDir: process.env.MOGE_DATA_DIR || path.join(app.getPath('userData'), 'workspace'), vault: safeStorage.isEncryptionAvailable() ? { encrypt: s => safeStorage.encryptString(s).toString('base64'), decrypt: s => safeStorage.decryptString(Buffer.from(s, 'base64')) } : undefined, fetchImpl: (url, init) => net.fetch(url, init), proxyProvider: async () => { const proxy = await session.defaultSession.resolveProxy('https://chatgpt.com'); const match = proxy.match(/(?:^|;\s*)PROXY\s+([^;]+)/); return match ? `http://${match[1].trim()}` : ''; } });
    ...
    window.removeMenu();
    window.webContents.setWindowOpenHandler(({ url }) => { try { const target = new URL(url); if (target.protocol === 'https:' && ['platform.openai.com', 'developers.openai.com', 'learn.chatgpt.com', 'auth.openai.com', 'auth0.openai.com', 'chatgpt.com'].includes(target.hostname)) void shell.openExternal(url); } catch { /* Unsupported URLs stay blocked. */ } return { action: 'deny' }; });
    ...
    window.on('close', event => {
      ...
      const answer = dialog.showMessageBoxSync(window, { type: 'question', buttons: ['继续创作', '停止并退出'], defaultId: 0, cancelId: 0, message: '还有生成任务正在进行', detail: '退出会停止后续生成。已完成的内容会保留，已提交的请求仍可能消耗额度或产生 API 费用。' });
  ...
  app.on('before-quit', () => { if (service) { for (const controller of service.jobs.controllers.values()) controller.abort(); service.ai.close(); service.server.close(); } });
}
